...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  <xsl:template match="readerlist">
    <xsl:element name="div">
      <xsl:attribute name="class">readerlist</xsl:attribute>
      <xsl:for-each select="/buildinfo/document/set/reader">
        ...
        <xsl:element name="div">
          ...
          <xsl:element name="h1">
            <xsl:element name="a">
              <xsl:attribute name="class">info homepage</xsl:attribute>
          ...
          <xsl:element name="span">
            <xsl:attribute name="class">label platform</xsl:attribute>
          ...
          <xsl:for-each select="platform">
            <xsl:element name="a">
              <xsl:attribute name="class">info platform</xsl:attribute>
              <xsl:attribute name="href">
                <xsl:value-of select="installer"/>
              </xsl:attribute>
              <xsl:value-of select="name"/>
            </xsl:element>
          </xsl:for-each>
