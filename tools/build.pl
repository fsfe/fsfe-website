...
unless ( $threads = $opts{t} ) { $threads = 1;}
...
#
my @dirs = File::Find::Rule->directory()->in( $opts{i} );

print STDERR "Resetting path for global\n" unless $opts{q};
rmtree( $opts{o} . '/' . "global" ) unless ( $opts{u} || $opts{n} );
my @paths = map { $opts{o} . "/global/" . $_ } grep( !/^\.\.?$/, @dirs );
foreach (@paths) {
    print "Creating $_\n" if $opts{d};
...
#
my @files = File::Find::Rule->file()->in( $opts{i} );
...
foreach my $i ( 1 .. $threads ) {
    ...
    socketpair( $procs[$i]{child}, $procs[$i]{parent},
...
sub process {
    ...
    my $backup = $dom->createElement("menuset");
    $root->appendChild($backup);
    clone_document( $backup, $opts{i} . "/tools/menu-global.xml" );

    print STDERR "global " unless $opts{q};
    ...
    while ( my ( $lang, undef ) = each %languages ) {
        ...
        unless ( -f $source ) {
            ...
            if ( -f "$opts{i}/$file.en.xhtml" ) {
                ...
                . $root->getAttribute("original")
                . ".xhtml" )
            {
                $document->setAttribute( "language",
                    $root->getAttribute("original") );
                $source =
                    "$opts{i}/$file."
                  . $root->getAttribute("original")
        ...
        if (
            $sourcedoc->documentElement->getAttribute("external") eq "yes" )
        {
            $document->setAttribute( "external", "yes" );
        }
        if ( $sourcedoc->documentElement->getAttribute("newsdate") ) {
            $document->setAttribute( "newsdate",
                $sourcedoc->documentElement->getAttribute("newsdate") );
        }
        if ( $sourcedoc->documentElement->getAttribute("type") ) {
            $document->setAttribute( "type",
                $sourcedoc->documentElement->getAttribute("type") );
        }
        ...
        #
        my $originalsource = "$file.en.xhtml";
        if ( ( stat("$opts{i}/$originalsource") )[9] >
            ( stat($source) )[9] + 7200
        ...
        foreach ( $root->getElementsByTagName("textset") ) {
            $root->removeChild($_);
        }
        ...
        if ( -f $opts{i} . "/fundraising.$lang.xml" ) {
            ...
            clone_document( $fundraisingdoc,
                $opts{i} . "/fundraising.$lang.xml", 1 );
        }
        elsif ( -f $opts{i} . "/fundraising.en.xml" ) {
            my $fundraisingdoc = $dom->createElement("fundraising");
            $root->appendChild($fundraisingdoc);
            clone_document( $fundraisingdoc,
                $opts{i} . "/fundraising.en.xml", 1 );
        }
        ...
        if ( -f "$opts{i}/$file.rss.xsl" ) {
            my $style_doc_rss =
              $parser->parse_file("$opts{i}/$file.rss.xsl");
            my $stylesheet_rss =
              $xslt_parser->parse_stylesheet($style_doc_rss);
            my $results_rss = $stylesheet_rss->transform($dom);
            $stylesheet_rss->output_file( $results_rss,
                "$opts{o}/global/$file.$lang.rss" )
              unless $opts{n};
        }
        ...
        if ( -f "$opts{i}/$file.ics.xsl" ) {
            my $style_doc_ics =
              $parser->parse_file("$opts{i}/$file.ics.xsl");
            my $stylesheet_ics =
              $xslt_parser->parse_stylesheet($style_doc_ics);
            my $results_ics = $stylesheet_ics->transform($dom);
            $stylesheet_ics->output_file( $results_ics,
                "$opts{o}/global/$file.$lang.ics" )
              unless $opts{n};
        }
        ...
        foreach ( $results->documentElement->getElementsByTagName("a") ) {
            ...
            if ( ( $href !~ /^http/ ) && ( $href !~ /^#/ ) ) {
                ...
                # process URL
                if (   ( $href =~ /\.(html|rss|ics)$/ )
                    && ( $href !~ /\.[a-z][a-z]\.(html|rss|ics)$/ ) )
                {
                    $href =~ s/\.(html|rss|ics)$/\.$lang.$1/;
                }
                elsif ( -d $opts{i} . "/$href" ) {
        ...
        print "Writing: $opts{o}/global/$file.$lang.html\n" if $opts{d};
        ...
        unless ( $opts{n} ) {
            ...
            #         print "$st\n\n\n\n\n\n\n\n\n";
            open( OUT, ">$opts{o}/global/$file.$lang.html" );
            binmode OUT, ":utf8";
            print OUT $st;
            close(OUT);
        }

        #$stylesheet->output_file($results, "$opts{o}/global/$file.$lang.html") unless $opts{n};
        # Add foo.html.xx link which is used by Apache's MultiViews option when
        # a user enters foo.html as URL.
        link(
            "$opts{o}/global/$file.$lang.html",
            "$opts{o}/global/$file.html.$lang"
        ) unless $opts{n};
    }
    print STDERR "\n" unless $opts{q};
}
...
my @dirs = File::Find::Rule->directory()->in( $opts{o} . "/global" );
foreach (@dirs) {
    my $base = basename($_);
    while ( my ( $lang, undef ) = each %languages ) {
...
foreach (
    grep( !/\.sources$/,
        grep( !/\.xsl$/, grep( !/\.xml$/, grep( !/\.xhtml$/, @files ) ) ) )
  )
{
    if ( -f "$opts{i}/$_" && !$opts{n} ) {
        link( "$opts{i}/$_", "$opts{o}/global/$_" );
    }
}
#
