...
Promise.all([
  ...
  fetch(`strings.${document.documentElement.lang}.js`).then((response) =>
    response.ok ? document.documentElement.lang : "en",
  ),
]).then(([pages, language]) => {
  ...
  importPromise.then((texts) => {
    if (searchString) {
      ...
      let matches = index
        .search(searchString)
        .filter((p) => locals.some((local) => p.ref.includes(local + ".html")));
