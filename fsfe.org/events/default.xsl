...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="include-events">
    ...
    <xsl:for-each select="/buildinfo/document/set/event[         not(           (             $group = 'past'             and             translate(substring(@end,1,10),'-','') &gt;= translate(/buildinfo/@date,'-','')           )           or           (             $group = 'current'             and             (               translate(substring(@start,1,10),'-','') &gt; translate(/buildinfo/@date,'-','')               or               translate(substring(@end,1,10),'-','') &lt; translate(/buildinfo/@date,'-','')             )           )           or           (             $group = 'future'             and             translate(substring(@start,1,10),'-','') &lt;= translate(/buildinfo/@date,'-','')           )         )       ]">
      <!-- Define sort order -->
