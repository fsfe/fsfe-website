...
echo "$files" | while read -r fullname priority; do
	...
	echo "zz zz zz zz zz zz zz zz zz"
done | sort -t' ' -k 1,1 -k 4,4 -k 3,3 |
	while read -r lang_short lang_long page priority originaldate original_url original_version translation_url translation_version; do
