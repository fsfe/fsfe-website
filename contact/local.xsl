...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="dynamic-content">
    <xsl:for-each select="/html/set/contact">
      ...
      <xsl:element name="p">
        <xsl:choose>
          <xsl:when test="count(/html/set/person[count(country[text()=$country])>0 and @teams='main'])>0">
            <xsl:value-of select="/html/text[@id='members']" />
            <xsl:text> </xsl:text>
            <xsl:for-each select="/html/set/person[count(country[text()=$country])>0 and @teams='main']">
