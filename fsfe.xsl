  ...
  <xsl:template match="h1">
    ...
    <xsl:if test="string(/buildinfo/document/@newsdate) and
      ...
      <xsl:element name="a">
        ...
        <xsl:attribute name="href">https://flattr.com/submit/auto?user_id=fsfe&amp;url=http://fsfe.org/<xsl:value-of select="/buildinfo/@filename" />.html&amp;title=<xsl:value-of select="document(concat(string(/buildinfo/@filename), '.en.xhtml'))/html/head/title" />&amp;description=<xsl:value-of select="document(concat(string(/buildinfo/@filename), '.en.xhtml'))/html/body/p[@newsteaser]" />&amp;tags=<xsl:for-each select="document(concat(string(/buildinfo/@filename), '.en.xhtml'))/html/tags/tag"><xsl:value-of select="node()" />,</xsl:for-each>&amp;category=text</xsl:attribute>
