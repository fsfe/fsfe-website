...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:str="http://exslt.org/strings" version="1.0" extension-element-prefixes="str">
  ...
  <xsl:template match="/buildinfo/document">
    ...
    <xsl:call-template name="nl"/>
    <xsl:text>PRODID:fsfe.org/events/events.ics.xsl</xsl:text>
    <xsl:call-template name="nl"/>
    <!-- Future events -->
    <xsl:for-each select="/buildinfo/document/set/event[           translate(substring(@start,1,10), '-', '') &gt;= translate(/buildinfo/@date, '-', '')         ]">
