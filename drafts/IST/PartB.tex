...
Any given machine running SICHA can host a list of
...
package SICHA-single implements a single machine setup
and depends on exim, cvs and bind. There can also be a
SICHA-multi-cvs, SICHA-multi-exim, SICHA-multi-bind,
...
project. This source will contain three types
of packages: copy of existing Debian packages slightly modified
for the needs of SICHA, packages of unfinished software written
for SICHA, packages whose sole purpose is to bind a set of 
packages together and configure them. 
\\
The software specific to SICHA will be packaged at a very
early stage. Basically the package will be done whenever the
...
organized way. Finally, since SICHA is a collection of
...
down-time, SICHA is commited to provide an upgrade
...
between distinct SICHA platforms.

\chapter*{B4. Contribution to program/Key action objectives}
\addcontentsline{toc}{chapter}{B4. Contribution to Key action objectives}
...
%% DRAFT, be nice :-) . GM
The SICHA project completely follows the '' Take up Action `` second objective.
Indeed, it is designed to allow anybody even with limited ressources to install
a development for free software. Therefore, even smallest projects will have a
complete platform which will use the generic interfaces available on bigger
sites. Furthermore, free software related organizations will have a complete
packaged solution to provide all the needed tools for development. Since the
project is completely distributed, it will have little architecture
requirements and then will be easy to integrate into an existing network.
\\ \\

Furthermore, the availabiliy of such software that makes high quality free
software development a reachable reality for everyone had already encouraged
free software development. Platforms like Savannah or SourceForge have allowed
a huge number of people to launch their project and to simplify its diffusion.
We are confident that the availability of a free and powerful platform in most
European language will encourage the creation of lots of hosting facilities in
Europe and then encourages Free software development.
...
hosting plateform. The frontend integrates different functionnalities that are
available to project developpers and administrators and linked them with each
other. The backend is mainly implemented with scripts. \\
...
Savannah (http://savannah.gnu.org/), VHFFS (http://tuxfamily.org/), and
...
well in the Libre software community than in the proprietary software industry.
\\ 
...
functionnalities. Even if the access is remote, all data is centralized as one
...
Internationalization lacks on most platforms, the only one that provides it had
recently turned proprietary. Thus this effort is not available.
