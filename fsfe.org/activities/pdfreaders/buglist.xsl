...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="dynamic-content">
    ...
    <xsl:for-each select="/buildinfo/document/set/buglist">
      ...
      <!-- Heading -->
      <xsl:element name="h2"><xsl:attribute name="id"><xsl:value-of select="$country"/></xsl:attribute><xsl:value-of select="/buildinfo/document/set/country[@id=$country]"/><xsl:variable name="nbsolved" select="count( /buildinfo/document/set/buglist[@country=$country]/bug[@closed != ''] )"/><xsl:variable name="nbinst" select="count( /buildinfo/document/set/buglist[@country=$country]/bug )"/>
      ...
      <xsl:element name="table">
        <xsl:element name="tr">
          <xsl:element name="th">
            <xsl:value-of select="/buildinfo/document/text[@id='institution-name']"/>
          </xsl:element>
          ...
          <xsl:element name="th">
            <xsl:value-of select="/buildinfo/document/text[@id='opened']"/>
          </xsl:element>
          <xsl:element name="th">
            <xsl:value-of select="/buildinfo/document/text[@id='closed']"/>
          </xsl:element>
          <xsl:element name="th">
            <xsl:value-of select="/buildinfo/document/text[@id='name']"/>
          </xsl:element>
          <xsl:element name="th">
            <xsl:value-of select="/buildinfo/document/text[@id='group']"/>
          </xsl:element>
          <xsl:element name="th">
            <xsl:value-of select="/buildinfo/document/text[@id='closedby']"/>
          </xsl:element>
          <xsl:element name="th">
            <xsl:value-of select="/buildinfo/document/text[@id='comment']"/>
          </xsl:element>
        </xsl:element>
        <!-- Table rows -->
        <xsl:for-each select="bug">
          <xsl:element name="tr">
            ...
            <xsl:comment>
              <xsl:element name="td">
                <xsl:value-of select="translate(@institution-address, '-', '–' )"/>
              </xsl:element>
            </xsl:comment>
            <xsl:element name="td">
